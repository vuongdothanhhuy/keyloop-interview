...
|---|---|
| Angular ^22.0.6 (resolves to 22.0.6) | Verified as the latest stable release; zoneless change detection and Vitest are its actual `ng new` defaults, confirmed directly against `@schematics/angular`'s own schema — not experimental opt-ins. |
...
| `json-server` ^0.17.4 + `@faker-js/faker` | A real HTTP boundary with persisted mutations, as opposed to `in-memory-web-api`, MSW, or a fake interceptor — this matters because the HTTP logging interceptor and the latency/error-injection middleware both need genuine network round-trips to be meaningful. Kept on the last stable pre-1.0 release line, not the in-progress `1.0.0-beta.x` rewrite, for reproducibility. |
...
7. **Angular's `@angular/*` packages are caret-ranged (`^22.0.x`) and currently resolve to 22.0.6** (the latest stable major as of this build), with `@ngrx/signals` dropped in favor of a plain `signal()`/`@Service()` store specifically because `@ngrx/signals` doesn't yet support Angular 22.
8. **`json-server` kept on the stable `0.17.x` line (resolves to `0.17.4`)**, not the in-progress `1.0.0-beta.x` rewrite, for reproducibility in a graded submission.
...
11. **"Log Action" is available on every vehicle, not restricted to aging stock**, even though the PDF's requirement 3 wording is "log ... a status or proposed action for each aging vehicle." The literal reading would gate the button/link on `isAging`; this build deliberately doesn't, since a manager plausibly wants to log a proactive action (e.g. a planned price reduction) on a vehicle *before* it crosses the 90-day threshold, and restricting the action doesn't serve any requirement the assessment actually evaluates — it only removes flexibility. Raised during an external code review of the PR and resolved as a documented assumption rather than left as a silent, undecided gap.
